...
print(
  ggplot(df, aes(x = num_copies)) +
    geom_line(aes(y = sample_mean), color = "blue") +   # Sample mean line
...
# Function to calculate the variance of the sample means
calculate_variance <- function(num_copies, n_trials, p) {
  sample_variances <- numeric(num_copies)

  for (i in 1:num_copies) {
    sample_data <- rbinom(i, size = n_trials, prob = p)
